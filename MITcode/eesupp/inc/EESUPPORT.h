...
C     | EESUPPORT.h                                              |
C     *==========================================================*
C     | Support data structures for the MITgcm UV ``execution    |支持MITgcm UV“执行环境”代码的数据结构。这些数据应该是执行环境例程的私有数据。需要通过数值模型直接访问的数据以EEPARAMS.h格式存储。
...
C     ERROR_HEADER        - String which prefixes error messages 以错误消息为前缀的字符串
      CHARACTER*(*) ERROR_HEADER
      PARAMETER ( ERROR_HEADER = ' *** ERROR ***' )
C     PROCESS_HEADER      - String which prefixes processor number 以处理器号为前缀的字符串
      CHARACTER*(*) PROCESS_HEADER
      PARAMETER ( PROCESS_HEADER = 'PID.TID' )

C     MAX_NUM_COMM_MODES - Maximum number of communication modes  最大通信模式数
C     COMM_NONE       - No edge communication   无边缘通信
C     COMM_MSG        - Use messages to communicate edges   使用消息来边缘通信
C     COMM_PUT        - Use put to communicate edges   使用put边缘通信
C     COMM_GET        - Use get to communicate edges   使用get边缘通信
...
C     Tile identifiers  tile的id
C     Tiles have a number that is unique over the global domain. 磁贴的编号在全局域中是唯一的。不存在的磁贴将其编号设置为空\u磁贴
C     A tile that is not there has its number set to NULL_TILE
      INTEGER NULL_TILE
      PARAMETER ( NULL_TILE = -1 )


C--   COMMON /EESUPP_C/ Execution environment support character variables  执行环境支持字符变量
C     myProcessStr - String identifying my process number         字符串，标识我的进程号
      COMMON /EESUPP_C/ myProcessStr
      CHARACTER*128 myProcessStr

C--   COMMON /EESUPP_L/ Execution environment support logical variables   执行环境支持逻辑变量
...
C     notUsingXPeriodicity - Flag indicating no X/Y boundary wrap around 表示无X/Y边界环绕的标志
C     notUsingYPeriodicity   This affects the communication routines but
C                            is generally ignored in the numerical model 这会影响通信例程，但在数值模型代码中通常会被忽略
C                            code.
C     threadIsRunning, threadIsComplete - Flags used to check for correct behaviour  用于检查多线程代码的正确行为的标志
...
C--   COMMON /EESUPP_I/ Parallel support integer globals  并行支持整数全局
C     pidW   -  Process  ID of neighbor to West   西邻进程ID
C     pidE   -           ditto             East   东邻进程ID
C     pidN   -           ditto             North  北邻进程ID
C     pidS   -           ditto             South  南邻进程ID
C              Note: pid[XY] is not necessairily the UNIX
C                    process id - it is just an identifying
C                    number.
C     myPid  - My own process id   我自己的进程id
C     nProcs - Number of processes 进程数
C     westCommunicationMode  - Mode of communication for each tile face  每个瓷砖表面的通信模式
...
#ifdef ALLOW_USE_MPI 
C--   Include MPI standard Fortran header file  包含MPI标准Fortran头文件
#include "mpif.h"
#define _mpiTRUE_  1
#define _mpiFALSE_ 0

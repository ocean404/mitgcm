...
C     纬向表面风应力    > 0 for increase in uVel, which is west to
C                      east for cartesian and spherical polar grids
C                  Typical range: -0.5 < ustress < 0.5
C                  Input field
C        
C     vstress   :: Meridional surface wind stress in N/m^2
C   经向表面风应力    > 0 for increase in vVel, which is south to
C                      north for cartesian and spherical polar grids
C                  Typical range: -0.5 < vstress < 0.5
C                  Input field
C        
C     hflux     :: Net upward surface heat flux including shortwave in W/m^2
C    净向上表面热通量（包括短波）  hflux = latent + sensible + lwflux + swflux
C                  > 0 for decrease in theta (ocean cooling)
C                  Typical range: -250 < hflux < 600
C                  Input field
C     
C     sflux     :: Net upward freshwater flux in m/s
C  净向上淡水通量    sflux = evap - precip - runoff
C                  > 0 for increase in salt (ocean salinity)
C                  Typical range: -1e-7 < sflux < 1e-7
C                  Input field
C        
C     swflux    :: Net upward shortwave radiation in W/m^2
C   净向上短波辐射     swflux = - ( swdown - ice and snow absorption - reflected )
C                  > 0 for decrease in theta (ocean cooling)
C                  Typical range: -350 < swflux < 0
C                  Input field
C       
C     uwind     :: Surface (10-m) zonal wind velocity in m/s
C   海面10m纬向风速     > 0 for increase in uVel, which is west to
C                      east for cartesian and spherical polar grids
C                  Typical range: -10 < uwind < 10
C                  Input or input/output field
C     
C     vwind     :: Surface (10-m) meridional wind velocity in m/s
C  海面10m经向风速   > 0 for increase in vVel, which is south to
C                      north for cartesian and spherical polar grids
C                  Typical range: -10 < vwind < 10
C                  Input or input/output field
C        
C     wspeed    :: Surface (10-m) wind speed in m/s
C  海面10m风速大小   >= 0 sqrt(u^2+v^2)
C                  Typical range: 0 < wspeed < 10
C                  Input or input/output field
C        
C     atemp     :: Surface (2-m) air temperature in deg K
C  海面2m气温     Typical range: 200 < atemp < 300
C                  Input or input/output field
C        
C     aqh       :: Surface (2m) specific humidity in kg/kg
C  海面2m比湿      Typical range: 0 < aqh < 0.02
C                  Input or input/output field
C       
C     lwflux    :: Net upward longwave radiation in W/m^2
C  净向上长波辐射  lwflux = - ( lwdown - ice and snow absorption - emitted )
C                  > 0 for decrease in theta (ocean cooling)
C                  Typical range: -20 < lwflux < 170
C                  Input field
C        
C     evap      :: Evaporation in m/s
C     蒸发       > 0 for increase in salt (ocean salinity)
C                  Typical range: 0 < evap < 2.5e-7
C                  Input, input/output, or output field
C       
C     precip    :: Total Precipitation (rain+snow) in m/s of liquid water
C   总降水（雨和雪）     > 0 for decrease in salt (ocean salinity)
C                  Typical range: 0 < precip < 5e-7
C                  Input or input/output field
C       
C     snowprecip :: snow precipitation in m/s of equivalent liquid water
C  降雪量（以液态水为单位）  > 0 for decrease in salt (ocean salinity)
C                  Typical range: 0 < precip < 5e-7
C                  Input or input/output field
C       
C     runoff    :: River and glacier runoff in m/s
C   河流和冰川径流    > 0 for decrease in salt (ocean salinity)
...
C  净向上盐通量   > 0 for decrease in SSS.
...
C  向下短波辐射   > 0 for increase in theta (ocean warming)
C                  Typical range: 0 < swdown < 450
C                  Input/output field
C        
C     lwdown    :: Downward longwave radiation in W/m^2
C    向下长波辐射   > 0 for increase in theta (ocean warming)
...
C   海洋感热通量    > 0 for increase in theta (ocean warming)
C        
C     hl        :: latent   heat flux into ocean in W/m^2
C   海洋潜热通量   > 0 for increase in theta (ocean warming)
C        
...
C     wStress   :: wind-stress magnitude [Pa=N/m^2], @ grid-cell center   风应力大小
C     sh        :: wind-speed [m/s] (always larger than uMin)             风速大小，始终大于uMin
 注意这里的公共变量定义     COMMON /exfl_wind_r/ wStress, cw, sw, sh
